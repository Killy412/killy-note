...
>   - 老年代,采用的回收算法是标记处理算法
>
> - <font color="red">**方法区(永久代):存储虚拟机加载的类信息,常量,静态变量,所有线程共享的区域,是一片连续的堆空间,占用空间是本地内存**</font>配置永久代大小 **-XX:MaxPermSize** 默认是64M,64位JVM是85M,方法区相关配置如下
>
...
类加载并不是把所有类在开始时都加载到jvm中,而是在需要的时候加载对应的类,分为主动加载和被动加载

- 主动引用: 调用`Class.forName()`和`ClassLoader.loadClass()`
  - 遇到new/getstatic/putstatic/invokestatic指令时加载类
- 被动引用
  - //TODO
...
3. **初始化,执行类构造器(\<clinit\>)的过程**,如果该类具有超类,则先初始化超类,执行static初始化器和static初始化块.
...
- 软引用: 在发生oom之前还会再进行一次gc,这次gc会把软引用对象回收掉,如果内存依然不够,会发生oom.
- 弱引用: 对象可以存活到下次gc之前,一旦发生gc,就会被回收.
- 虚引用: 幽灵引用. 对对象没有任何影响. 在回收的时候,jvm会收到此对象被回收的通知.
...
- Shenandoah
  ...
  java12引入. 与G1收集器类似,基于Region设计的垃圾回收器.停顿时间和堆大小没有关系.停顿时间与ZGC接近.
